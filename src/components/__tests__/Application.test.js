...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Check that the Form with preset info is shown
    const nameInput = getAllByTestId(container, "student-name-input")[0]
    expect(nameInput.value).toBe("Archie Cohen");
    ...
    // 11 Check that the DayListItem with the text "Monday" did not altered the "1 spot remaining".
    const day = getAllByTestId(container, "day").find(day =>
      queryByText(day, "Monday")
    );
    expect(getByText(day, "1 spot remaining")).toBeInTheDocument();
